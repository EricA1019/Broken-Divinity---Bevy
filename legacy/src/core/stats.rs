...
impl PlayerProgression {
    pub fn new_game() -> Self {
        use crate::core::config::game_balance;
        let cfg = game_balance();
        let mut progression = Self::default();

        progression.virtues.insert(VirtueId::Temperance, VirtueState { rank: cfg.virtues.temperance });
        progression.virtues.insert(VirtueId::Justice, VirtueState { rank: cfg.virtues.justice });
        progression.virtues.insert(VirtueId::Prudence, VirtueState { rank: cfg.virtues.prudence });
        progression.virtues.insert(VirtueId::Fortitude, VirtueState { rank: cfg.virtues.fortitude });
        progression.virtues.insert(VirtueId::Thumos, VirtueState { rank: cfg.virtues.thumos });
        progression.virtues.insert(VirtueId::Metis, VirtueState { rank: cfg.virtues.metis });

        progression.proficiencies.insert(
            ProficiencyId::MeleeTraining,
            ProficiencyState::new(cfg.proficiencies.melee_training),
        );
        progression.proficiencies.insert(
            ProficiencyId::RangedTraining,
            ProficiencyState::new(cfg.proficiencies.ranged_training),
        );
        progression.proficiencies.insert(
            ProficiencyId::QuietMovement,
            ProficiencyState::new(cfg.proficiencies.quiet_movement),
        );
        progression.proficiencies.insert(
            ProficiencyId::Medicine,
            ProficiencyState::new(cfg.proficiencies.medicine),
        );
        progression.proficiencies.insert(
            ProficiencyId::Repair,
            ProficiencyState::new(cfg.proficiencies.repair),
        );
        progression.proficiencies.insert(
            ProficiencyId::Ritecraft,
            ProficiencyState::new(cfg.proficiencies.ritecraft),
        );
...
mod tests {
    ...
    #[test]
    fn test_canonical_model_drives_runtime_combat_skills() {
        // Phase C: Confirm the canonical VirtueId/ProficiencyId model is the
        // runtime source of truth. SkillId pilot proxies are derived views.
        // Legacy SkillId variants (Toughness, Stealth, Awareness, Repair,
        // Leadership) must not be consumed by production combat paths.
        let progression = PlayerProgression::new_game();

        // Canonical model produces meaningful values
        assert!(progression.virtue_rank(VirtueId::Fortitude) > 0);
        assert!(progression.proficiency_rating(ProficiencyId::MeleeTraining) > 0);

        // Action ratings derive from virtue + proficiency, not from SkillId
        let melee_ar = progression.action_rating(
            VirtueId::Thumos, ProficiencyId::MeleeTraining, 0, 0,
        );
        let ranged_ar = progression.action_rating(
            VirtueId::Prudence, ProficiencyId::RangedTraining, 0, 0,
        );
        assert!(melee_ar > 0, "Melee action rating should be positive");
        assert!(ranged_ar > 0, "Ranged action rating should be positive");

        // Enemy defense DV uses Metis + QuietMovement
        let defense_dv = progression.enemy_attack_dv();
        assert!(defense_dv > 0, "Defense DV should be positive");

        // These values are deterministic (same seed = same result)
        let p2 = PlayerProgression::new_game();
        assert_eq!(progression.virtue_rank(VirtueId::Temperance), p2.virtue_rank(VirtueId::Temperance));
        assert_eq!(
            progression.proficiency_rating(ProficiencyId::Medicine),
            p2.proficiency_rating(ProficiencyId::Medicine),
        );
    }
}
