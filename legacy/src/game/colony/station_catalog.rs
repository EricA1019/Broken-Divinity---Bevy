...
pub fn all_station_defs() -> &'static [StationDef] {
    STATIONS.get_or_init(|| {
        let ron_defs: Vec<RonStationDef> = ron::from_str(STATIONS_RON_SOURCE)
            .expect("Failed to parse embedded stations.ron");
        ron_defs
            .into_iter()
            .map(|r| StationDef {
                ...
                build_cost: r.build_cost.into_iter().map(|c| (c.kind, c.amount)).collect(),
...
mod tests {
    ...
    fn test_stations_loads_and_has_expected_count() {
        ...
        assert_eq!(defs.len(), 10, "All 10 station types must be defined in stations.ron");
    }
    ...
    fn test_ammo_press_consumes_scrap() {
        ...
        let input = recipe.input.as_ref().expect("AmmoPress should consume scrap");
