...
pub fn resolve_player_melee(
        ...
        enemy_q.iter_mut()
    {
        ...
        if check.fumble {
            log.push(
                format!("You fumble your attack on {}!{}", enemy_name.name, action_breakdown),
                LogColor::Miss,
                game_time.turn,
            );
...
mod tests {
    ...
    fn test_enemy_melee_check_breakdown_formats_expected_terms() {
        let progression = PlayerProgression::new_game();
        let breakdown =
            enemy_melee_check_breakdown(18, &progression, &PlayerPerks::default(), 11);
    ...
    fn test_enemy_melee_stops_after_fatal_hit() {
        ...
        assert!(hit_entries[0].text.contains("[ATK 10000 vs DV 11 = MET 5 + QUI 6]"));
    ...
    fn test_enemy_melee_logs_progression_defense_breakdown() {
        ...
        assert!(hit_entry.text.contains("[ATK 10000 vs DV 11 = MET 5 + QUI 6]"));
    }
