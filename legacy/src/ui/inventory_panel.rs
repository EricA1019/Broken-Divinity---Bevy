//! Toggle-able inventory panel — press I or Tab to open/close.
...
/// Toggle inventory visibility when I or Tab is pressed.
pub fn toggle_inventory(keys: Res<ButtonInput<KeyCode>>, mut open: ResMut<InventoryOpen>) {
    let toggled = keys.just_pressed(KeyCode::KeyI) || keys.just_pressed(KeyCode::Tab);
    if toggled {
        open.0 = !open.0;
    }
}
