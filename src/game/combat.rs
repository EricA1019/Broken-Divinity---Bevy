...
mod tests {
            ...
            for skill in [0, 25, 50, 100, 200] {
                for &mods in &[-50, -25, 0, 25, 50] {
                    for &dv in &[-50, -25, 0, 25, 50] {
                        ...
                        assert!(result.roll >= 1 && result.roll <= 100, "roll {} out of range", result.roll);
                    }
                }
            }
        }
    ...
    fn invariant_fumble_only_on_100() {
        for seed in 0u64..500 {
            ...
            for skill in [0, 25, 50, 100] {
                ...
                assert_eq!(result.fumble, result.roll == 100, "fumble must be true iff roll == 100 at seed={}", seed);
            }
        }
    }
    ...
    fn invariant_critical_implies_success() {
        for seed in 0u64..500 {
            ...
            for skill in [0, 25, 50, 100, 200] {
                let result = roll_check(skill, 0, 0, &mut rng);
                if result.critical {
                    assert!(result.success, "critical must imply success at seed={} skill={}", seed, skill);
                }
            }
        }
    }
            ...
            for skill in [0, 25, 50, 100] {
                for &mods in &[-30, 0, 30] {
                    for &dv in &[-30, 0, 30] {
                        ...
                        assert_eq!(result.target_number, expected, "formula violated at seed={}", seed);
                    }
                }
            }
        }
                ...
                for skill in [0u32, 10, 50, 99] {
                    for ar in [0, 10, 50] {
                        for &crit in &[false, true] {
                            for &dt in &[DamageType::Blunt, DamageType::Ballistic, DamageType::Celestial] {
                                let dmg = calc_damage(weapon, skill, ar, 0, dt, crit, &mut rng);
                                assert!(dmg >= 1, "damage must be >= 1, got {} for {:?} seed={}", dmg, dt, seed);
                            }
                        }
                    }
                }
    ...
    fn invariant_supernatural_ignores_ar() {
        for seed in 0u64..500 {
            ...
            let dmg_super = calc_damage(10, 15, 50, 0, DamageType::Celestial, false, &mut rng_super);
            assert!(dmg_super >= dmg_phys, "supernatural should >= physical vs high AR (phys={}, super={}) at seed={}", dmg_phys, dmg_super, seed);
        }
    }
}
