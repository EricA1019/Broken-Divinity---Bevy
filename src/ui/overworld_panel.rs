...
use crate::game::overworld::travel::{
    DEHYDRATION_EXPOSURE, DEHYDRATION_HP_DAMAGE, STARVATION_EXPOSURE, STARVATION_HP_DAMAGE,
    TravelState,
};
...
pub(crate) fn primary_overworld_cta_label() -> &'static str {
...
/// Draw overworld sidebar with current location, weather, travel progress, and nearby nodes.
                ...
                if has_supply_warning {
                    ...
                    if resources.food == 0 {
                        ui.colored_label(
                            danger,
                            format!(
                                "No food: -{} HP / +{} exposure each day",
                                STARVATION_HP_DAMAGE, STARVATION_EXPOSURE
                            ),
                        );
                    }
                    if resources.water == 0 {
                        ui.colored_label(
                            danger,
                            format!(
                                "No water: -{} HP / +{} exposure each day",
                                DEHYDRATION_HP_DAMAGE, DEHYDRATION_EXPOSURE
                            ),
                        );
                    }
                    ui.separator();
