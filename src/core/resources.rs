...
pub const COLONY_TICK_SECONDS: f32 = 1.0;
pub const TRAVEL_DAY_SECONDS: f32 = 1.0;
...
/// Real-time pacing for shelter simulation while in `AppState::Colony`.
...
/// Real-time pacing for overworld travel while `TravelState` exists.
#[derive(Resource, Debug, Clone)]
...
/// Shared in-memory placeholder atlas used until authored tile art exists.
#[derive(Resource, Debug, Clone)]
...
impl ShelterResources {
    /// Starting resources for a new game, loaded from balance tuning config.
    pub fn new_game() -> Self {
        let cfg = game_balance().shelter.clone();
        Self {
            food: cfg.starting_food,
            water: cfg.starting_water,
            scrap: cfg.starting_scrap,
            medicine: cfg.starting_medicine,
            ammo: cfg.starting_ammo,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_simulation_timers_default_to_one_second() {
        assert_eq!(
            ColonyTickTimer::default().0.duration().as_secs_f32(),
            COLONY_TICK_SECONDS
        );
        assert_eq!(
            TravelDayTimer::default().0.duration().as_secs_f32(),
            TRAVEL_DAY_SECONDS
        );
    }
}
