...
/// Advances the universal clock once for non-dungeon real-time simulation steps.
pub fn advance_game_time(mut game_time: ResMut<GameTime>) {
    game_time.turn += 1;
}
...
pub fn tick_sprint_cooldown(mut query: Query<&mut crate::core::abilities::SprintCooldown>) {
...
mod tests {
    use super::*;
    use crate::core::resources::{ColonyTickTimer, TravelDayTimer};
    use std::time::Duration;

    #[test]
    fn test_advance_game_time_increments_turn() {
        let mut app = App::new();
    ...
    #[test]
    fn test_colony_timer_finishes_after_full_interval() {
        let mut timer = ColonyTickTimer::default();

        timer.0.tick(Duration::from_secs_f32(0.5));
        assert!(!timer.0.just_finished());

        timer.0.tick(Duration::from_secs_f32(0.6));
        assert!(timer.0.just_finished());
    }

    #[test]
    fn test_travel_timer_finishes_after_full_interval() {
        let mut timer = TravelDayTimer::default();

        timer.0.tick(Duration::from_secs_f32(0.4));
        assert!(!timer.0.just_finished());

        timer.0.tick(Duration::from_secs_f32(0.7));
        assert!(timer.0.just_finished());
    }

    #[test]
    fn test_enemy_turn_max_frames_guard_is_defined() {
        // ENEMY_TURN_MAX_FRAMES prevents soft-locks: if an enemy has undrained
        // ActionBudget remaining, the guard force-advances after N frames.
        assert!(
            ENEMY_TURN_MAX_FRAMES > 0 && ENEMY_TURN_MAX_FRAMES <= 10,
            "ENEMY_TURN_MAX_FRAMES={} should be in [1, 10]",
            ENEMY_TURN_MAX_FRAMES,
        );
        // The guard activates when frame_counter >= ENEMY_TURN_MAX_FRAMES
        // At ENEMY_TURN_MAX_FRAMES=3, after 3 frames the counter equals the limit
        // and the EnemyTurn → WorldTick transition fires.
        assert!(
            ENEMY_TURN_MAX_FRAMES <= 3,
            "ENEMY_TURN_MAX_FRAMES={} should be ≤ 3 to prevent noticeable lag",
            ENEMY_TURN_MAX_FRAMES,
        );
    }
}
