...
impl FeedbackEvent {
    ...
    fn severity(self) -> LogColor {
        match self {
            Self::ColonyGateEnterHint
            | Self::EscOverworldBackHint
            | Self::EscHelpCloseHint => LogColor::Status,
...
pub fn blocked_action_message(action: &str, reason: &str, next_step: &str) -> String {
    format!("{action} failed: {reason}. Next: {next_step}.")
}

/// Ring-buffer resource holding the last N messages.
...
mod tests {
    ...
    const TEST_TURN_ZERO: u32 = 0;
    const TEST_TURN_ONE: u32 = 1;
    const TEST_TURN_THREE: u32 = 3;

    #[test]
    fn test_push_collapses_consecutive_duplicates_on_same_turn() {
        let mut log = GameLog::default();

        log.push("You miss.", LogColor::Miss, 12);
        log.push("You miss.", LogColor::Miss, 12);
        log.push("You miss.", LogColor::Miss, 12);
    ...
    #[test]
    fn test_push_keeps_distinct_turns_separate() {
        let mut log = GameLog::default();

        log.push("You miss.", LogColor::Miss, 12);
        log.push("You miss.", LogColor::Miss, 13);

        assert_eq!(log.entries().len(), 2);
        assert_eq!(log.entries()[0].count, 1);
        assert_eq!(log.entries()[1].count, 1);
    }

    #[test]
    fn warning_aggregation_throttles_repetition() {
        let mut log = GameLog::default();

        log.push_feedback(FeedbackEvent::TravelNoFood, TEST_TURN_ZERO);
        log.push_feedback(FeedbackEvent::TravelNoFood, TEST_TURN_ONE);

        assert_eq!(
            log.entries().len(),
            1,
            "Expected travel warning to be throttled within cooldown window"
        );

        log.push_feedback(FeedbackEvent::TravelNoFood, TEST_TURN_THREE);

        assert_eq!(
            log.entries().len(),
            2,
            "Expected travel warning to emit again after cooldown"
        );
    }

    #[test]
    fn severity_mapping_consistent_across_events() {
        assert_eq!(FeedbackEvent::TravelNoFood.severity(), LogColor::EnemyHit);
        assert_eq!(FeedbackEvent::TravelNoWater.severity(), LogColor::EnemyHit);
        assert_eq!(
            FeedbackEvent::RaidForecast {
                message: "raid forecast",
            }
            .severity(),
            LogColor::EnemyHit
        );
        assert_eq!(FeedbackEvent::ColonyGateEnterHint.severity(), LogColor::Status);
    }

    #[test]
    fn blocked_action_message_follows_expected_pattern() {
        let message = blocked_action_message(
            "Build Station",
            "Insufficient scrap",
            "Collect scrap before retrying",
        );

        assert_eq!(
            message,
            "Build Station failed: Insufficient scrap. Next: Collect scrap before retrying."
        );
    }
}
