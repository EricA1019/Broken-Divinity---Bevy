...
pub fn plugin(app: &mut App) {
    ...
    app.init_state::<turn::TurnPhase>()
        ...
        .init_resource::<resources::TravelDayTimer>()
        .init_resource::<gamelog::GameLog>()
        .init_resource::<perks::PendingPerkChoices>()
        .init_resource::<escape::LastActionTurn>()
        .add_systems(
            Startup,
            (camera::setup_camera, tilemap::init_placeholder_tile_atlas),
        )
        .add_systems(
            Update,
            (
        ...
        .add_systems(Update, escape::handle_escape_to_menu)
        .add_systems(Update, escape::check_player_death_universal)
        .add_systems(
            Update,
            escape::detect_stuck_player
                .run_if(in_state(state::AppState::Dungeon))
                .run_if(in_state(turn::TurnPhase::AwaitingInput)),
        )
        .add_systems(
            Update,
            escape::update_last_action_turn
                .run_if(in_state(state::AppState::Dungeon))
                .run_if(in_state(turn::TurnPhase::PlayerTurn)),
        );
}
