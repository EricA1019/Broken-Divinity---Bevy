...
const LOAD_RECAP_PRESSURE_RESOURCE_THRESHOLD: u32 = 2;
const LOAD_RECAP_DEFAULT_OBJECTIVE: &str =
    "Objective: Continue progression from your current state.";
const LOAD_RECAP_PRESSURE_OBJECTIVE: &str =
    "Objective: Stabilize food and water reserves immediately.";
const LOAD_RECAP_OVERWORLD_OBJECTIVE: &str =
    "Objective: Resume overworld travel and prepare for encounters.";
const LOAD_RECAP_COLONY_OBJECTIVE: &str =
    "Objective: Reach the shelter gate when ready to travel.";
const LOAD_SUCCESS_FEEDBACK: &str = "Load complete. Resuming your run.";
const SAVE_SUCCESS_FEEDBACK: &str = "Save complete. Returning to menu.";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum LoadGameError {
    MissingFile,
    ReadFailed,
    ParseFailed,
}
...
pub(crate) fn load_success_message() -> &'static str {
    LOAD_SUCCESS_FEEDBACK
}

pub(crate) fn save_success_message() -> &'static str {
    SAVE_SUCCESS_FEEDBACK
}

pub(crate) fn load_error_message(error: LoadGameError) -> &'static str {
    match error {
        LoadGameError::MissingFile => "Load failed: no save file found.",
        LoadGameError::ReadFailed => "Load failed: could not read save file.",
        LoadGameError::ParseFailed => "Load failed: save data is invalid.",
    }
}

pub(crate) fn save_error_message(error: SaveWriteError) -> &'static str {
    match error {
        SaveWriteError::SerializeFailed => "Save failed: could not serialize save data.",
        SaveWriteError::WriteFailed => "Save failed: could not write save file.",
    }
}
...
pub fn spawn_player_from_save_at(
    ...
    let progression = canonical_player_progression(player);
    ...
    progression.sync_pilot_combat_skill_proxies(&mut bundle.stats.skills);
    ...
    bundle.progression = progression;
...
fn canonical_player_progression(player: &SavePlayerState) -> PlayerProgression {
    let mut progression = if player.progression.is_empty() {
        PlayerProgression::from_legacy_skills(&player.stats.skills)
    } else {
        player.progression.clone()
    };
    progression.ensure_complete();
    progression
}

fn sync_player_progression_and_pilot_skill_proxies(
    player: &mut SavePlayerState,
    sync_pilot_skill_proxies: bool,
) {
    let progression = canonical_player_progression(player);
    if sync_pilot_skill_proxies {
        progression.sync_pilot_combat_skill_proxies(&mut player.stats.skills);
    }
    player.progression = progression;
}

fn load_state_has_pressure(save: &SaveGame) -> bool {
    save.colony.resources.food <= LOAD_RECAP_PRESSURE_RESOURCE_THRESHOLD
        || save.colony.resources.water <= LOAD_RECAP_PRESSURE_RESOURCE_THRESHOLD
        || save.colony.pending_raid_report.is_some()
}

fn load_recap_objective(save: &SaveGame) -> &'static str {
    if load_state_has_pressure(save) {
        return LOAD_RECAP_PRESSURE_OBJECTIVE;
    }

    match save.app_state.into_runtime_state() {
        AppState::Overworld => LOAD_RECAP_OVERWORLD_OBJECTIVE,
        AppState::Colony => LOAD_RECAP_COLONY_OBJECTIVE,
        _ => LOAD_RECAP_DEFAULT_OBJECTIVE,
    }
}

fn build_load_recap(save: &SaveGame) -> String {
    format!(
        "Load recap — Turn {} | Food: {} | Water: {} | {}",
        save.game_time.turn,
        save.colony.resources.food,
        save.colony.resources.water,
        load_recap_objective(save),
    )
}

fn queue_load_recap(commands: &mut Commands, save: &SaveGame) {
    let recap_text = build_load_recap(save);
    commands.queue(move |world: &mut World| {
        let turn = world.get_resource::<GameTime>().map_or(0, |time| time.turn);
        let Some(mut log) = world.get_resource_mut::<GameLog>() else {
            return;
        };
        log.push(recap_text.clone(), LogColor::System, turn);
    });
}
...
pub fn restore_persistent_run_resources(commands: &mut Commands, save: &SaveGame) {
    ...
    queue_load_recap(commands, save);
}
...
impl SaveGame {
    ...
    fn sync_derived_fields(&mut self) {
        let had_canonical_progression = !self.player.progression.is_empty();
        let sync_pilot_skill_proxies = had_canonical_progression || !self.has_legacy_flat_data();
        sync_player_progression_and_pilot_skill_proxies(
            &mut self.player,
            sync_pilot_skill_proxies,
        );
...
pub(crate) fn load_game_detailed() -> Result<SaveGame, LoadGameError> {
    let path = save_path();
    if !path.exists() {
        return Err(LoadGameError::MissingFile);
    }

    let raw = std::fs::read_to_string(path).map_err(|_| LoadGameError::ReadFailed)?;
    let save: SaveGame = serde_json::from_str(&raw).map_err(|_| LoadGameError::ParseFailed)?;
...
fn write_save(save: &SaveGame) -> bool {
    write_save_detailed(save).is_ok()
}

fn write_save_detailed(save: &SaveGame) -> Result<(), SaveWriteError> {
    let json = serde_json::to_string_pretty(save).map_err(|_| SaveWriteError::SerializeFailed)?;
    std::fs::write(save_path(), json).map_err(|_| SaveWriteError::WriteFailed)?;
    Ok(())
}
...
pub fn handle_save_and_quit(
    ...
    save.sync_derived_fields();
    match write_save_detailed(&save) {
        Ok(()) => {
            if let Some(log) = log.as_mut() {
                log.push(save_success_message(), LogColor::System, turn);
            }
            next_state.set(AppState::Menu);
        }
        Err(error) => {
            if let Some(log) = log.as_mut() {
                log.push(save_error_message(error), LogColor::Status, turn);
            }
        }
    }
}
...
mod tests {
    ...
    use crate::core::stats::{ProficiencyId, ProficiencyState, VirtueId, VirtueState};
    ...
    fn progression_with_custom_pilot_ratings() -> PlayerProgression {
        let mut progression = PlayerProgression::new_game();
        progression
            .virtues
            .insert(VirtueId::Thumos, VirtueState { rank: 4 });
        ...
        progression
            .virtues
            .insert(VirtueId::Prudence, VirtueState { rank: 3 });
        progression.proficiencies.insert(
        ...
        progression
            .virtues
            .insert(VirtueId::Metis, VirtueState { rank: 2 });
        progression.proficiencies.insert(
    ...
    #[test]
    fn test_normalize_loaded_mixed_save_prefers_canonical_progression_for_pilot_skill_proxies() {
        const LEGACY_TURN: u32 = 21;
        const LEGACY_MELEE_BASE: u32 = 99;
        const LEGACY_RANGED_BASE: u32 = 88;
        const LEGACY_TOUGHNESS_BASE: u32 = 77;
        const STALE_PROXY_BASE: u32 = 5;
        const STALE_PROXY_XP: u32 = 13;
        const STALE_PROXY_LEVEL: u32 = 4;

        let mut save = SaveGame::default();
        save.player = sample_player_state();
        save.player.progression = progression_with_custom_pilot_ratings();
        save.legacy_turn = LEGACY_TURN;
        save.legacy_player_melee_skill = LEGACY_MELEE_BASE;
        save.legacy_player_ranged_skill = LEGACY_RANGED_BASE;
        save.legacy_player_toughness_skill = LEGACY_TOUGHNESS_BASE;
        for skill in [SkillId::Melee, SkillId::Ranged, SkillId::Evasion] {
            save.player.stats.skills.insert(
                skill,
                SkillState {
                    base: STALE_PROXY_BASE,
                    xp: STALE_PROXY_XP,
                    level: STALE_PROXY_LEVEL,
                },
            );
        }
        ...
        assert_eq!(loaded.game_time.turn, LEGACY_TURN);
        assert_eq!(
            loaded.player.stats.skills[&SkillId::Melee].effective(),
            loaded
                .player
    ...
    #[test]
    fn test_spawn_player_from_save_syncs_pilot_skill_proxies_from_progression() {
        const STALE_PROXY_BASE: u32 = 3;
        const STALE_PROXY_XP: u32 = 8;
        const STALE_PROXY_LEVEL: u32 = 2;

        let mut player = sample_player_state();
        player.progression = progression_with_custom_pilot_ratings();
        for skill in [SkillId::Melee, SkillId::Ranged, SkillId::Evasion] {
            player.stats.skills.insert(
                skill,
                SkillState {
                    base: STALE_PROXY_BASE,
                    xp: STALE_PROXY_XP,
        ...
        let _ = world.run_system_once(move |mut commands: Commands| {
            spawn_player_from_save_at(&mut commands, &player, None);
        });

        let mut player_query = world.query::<(&CombatStats, &PlayerProgression)>();
        let (stats, progression) = player_query.single(&world).expect("spawned player");

        assert_eq!(
            stats.skills[&SkillId::Melee].effective(),
            progression.action_rating(VirtueId::Thumos, ProficiencyId::MeleeTraining, 0, 0)
                as u32
        );
        assert_eq!(
            stats.skills[&SkillId::Ranged].effective(),
            progression.action_rating(VirtueId::Prudence, ProficiencyId::RangedTraining, 0, 0)
                as u32
        );
        assert_eq!(
            stats.skills[&SkillId::Evasion].effective(),
            progression.enemy_attack_dv() as u32
        );
        assert_eq!(stats.skills[&SkillId::Melee].xp, 0);
        assert_eq!(stats.skills[&SkillId::Ranged].xp, 0);
        assert_eq!(stats.skills[&SkillId::Evasion].xp, 0);
    }
