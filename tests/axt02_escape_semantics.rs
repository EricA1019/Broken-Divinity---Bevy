...
fn one_shot_guidance_appears_once_per_context() {
    ...
    let first = engine.guidance(EscapeContext {
        modal_open: true,
        can_pause: true,
    });
    assert_eq!(first, EscapeGuidanceEvent::ShowHint);

    let second = engine.guidance(EscapeContext {
        modal_open: true,
        can_pause: true,
    });
...
fn acknowledgement_suppresses_future_hints() {
    ...
    let _ = engine.guidance(EscapeContext {
        modal_open: false,
        can_pause: true,
    });
    engine.acknowledge();

    let after_ack = engine.guidance(EscapeContext {
        modal_open: false,
        can_pause: true,
    });
