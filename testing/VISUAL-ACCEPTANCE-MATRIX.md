...
| Provisions pressure and dawn outlook | `provisions_pressure` | S,C,Y,G,P | `ui_development_contract_tests::provisions_show_stock_pressure_and_dawn_outlook_at_supported_profiles` | Green | Green | Green | Green | n/a | Green | Green | Open | D-20/D-22; UI plan 17 | Green unreviewed: structured exact gauge, pressure, dawn delta/result, profile composition, and map primacy pass; PTY/owner review remains open |
| Nearby Chronicle transition | `adjacent_interactable_entry` | S,C,G,T,P | `phase6_input::entering_adjacent_range_emits_one_deduplicated_nearby_hint` | Green | Green | n/a | Open | Green | Green | Green | Open | D-18; UI plan 17 | Green unreviewed: station/node entry aggregates to one focused fact plus a count, retains the complete target set, rearms after exit, and exposes truthful disabled Interact while unbound; PTY/owner review remains |
| Station/node/colonist Context | `context_category_matrix` | S,C,Y,G,T,P | `ui_development_contract_tests::nearby_station_context_is_complete_at_supported_profiles` | Red | Red | Open | Open | Open | Red | Red | Open | D-18/D-20; UI plan 17 | Red: default category rows pass, but active station/node/colonist state detail, its final composition, and bound-without-reducer Interact truth fail independently |
| Shared Cinder Rite identity | `selected_cinder_rite_reliquary_identity` | C,Y,G,P | `ui_development_contract_tests::selected_cinder_rite_identity_frames_colony_and_reusable_screens` | n/a | Green | Green | Green | n/a | Green | Green | Partial | Kernel Goal 5; selected UI style target | Green unreviewed: double shell/modal chrome, muted single-rule panels, responsive HP/AP tracks, and shared mode/command ribbons pass both automated profiles; live 80x24 and 60x20 PTY confirms normal/build-selection composition and restoration, while complete placement/profile and owner review remain open |
...
| Selected semantic roles resolve the exact Cinder Rite palette | `selected_cinder_rite_identity_frames_colony_and_reusable_screens` | UI1 | Green unreviewed |
| Representative screens contain a substantial closed double-line frame | `selected_cinder_rite_identity_frames_colony_and_reusable_screens` | UI1 | Green unreviewed: final buffer matrix passes; complete PTY/owner review remains |
| Ordinary panels use the shared muted single-rule Reliquary grammar | `outpost_panels_render_shared_chrome_at_supported_profiles`; `selected_cinder_rite_identity_frames_colony_and_reusable_screens` | UI1 | Green unreviewed: complete perimeters and title markers pass both profiles |
| Shared HP/AP meters retain exact values and partial tracks | `selected_cinder_rite_identity_frames_colony_and_reusable_screens`; observer mutation probe | UI1 | Green unreviewed: colony and combat pass with live-sized compact values |
| Mode and command ribbons preserve status and active controls | `selected_cinder_rite_identity_frames_colony_and_reusable_screens`; management footer contract | UI1/UI4 | Green unreviewed: both profiles and representative interaction states pass |
